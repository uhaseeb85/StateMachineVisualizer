...
interface AnimatedDemoProps {
  mode: 'stateMachine' | 'flowDiagram' | 'aiLogAnalysis' | 'logAnalyzer';
}

const AnimatedDemo: React.FC<AnimatedDemoProps> = ({ mode }) => {
  ...
  return (
    <>
      ...
      {isExpanded && typeof document !== 'undefined' && createPortal(renderModal(), document.body)}
    </>
  );
};
